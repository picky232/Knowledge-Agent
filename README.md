...
- 더 작은 모델(qwen3:4b): **오히려 52~79s로 훨씬 느리고** 자료없음 3/6로 악화(답변이 장황해짐). 삭제함
  - (2026-09-07 재검토) 기각은 맞지만 이유가 달랐다. "장황해진다"가 아니라
    **`think: false`가 qwen3:4b에서 추론을 끄지 못한다.** 직접 확인한 결과:

    | 설정 | `response` | `thinking` |
    |---|---|---|
    | `think: true` | 비어있음 | 추론 (정상 분리) |
    | `think: false` | **추론이 그대로 들어옴** | 비어있음 |

    끄는 게 아니라 분리만 해제한다. 그래서 답변 자리에 `"Okay, let's tackle this problem..."`로
    시작하는 영어 추론이 쏟아지고, `num_predict` 200을 다 쓰고도 결론에 닿지 못한다.
    Qwen3의 소프트 스위치 `/no_think`를 프롬프트에 넣어도 마찬가지였다. qwen3:8b는 같은
    플래그로 추론이 실제로 억제된다. 모델이 작아 얻는 이득보다 매 답변마다 버리는
    토큰이 훨씬 크므로 쓸 수 없다. 8b 30문항 대조군은 거부 2건 / 평균 9.71s였다.
...
## 체감 지연은 프롬프트를 읽는 시간이다 (2026-09-07)

채팅창은 토큰을 흘려보내므로 사용자가 기다리는 시간은 **첫 글자가 뜰 때까지**다.
그 시간이 무엇으로 채워져 있는지 재보니:

- **검색은 병목이 아니다.** 워밍 후 벡터 검색 0.003초, 질문 임베딩 0.058초.
  첫 호출의 1.1초는 임베딩 행렬을 만드는 비용인데, 채팅창은 서버가 상주하므로 한 번만 낸다.
- **생성 속도도 첫 글자와 무관하다.** 같은 모델이 10.9 tok/s일 때와 51.1 tok/s일 때
  첫 글자까지는 둘 다 4~6초였다. 첫 글자 앞에 있는 건 생성이 아니라 프롬프트 읽기다.
- **남는 건 프롬프트 길이뿐이다.** 근거 조각 다섯 개면 평균 798토큰.

조각 수별 토큰 (`kb prompt-size`, 질문 10개):
...
**조각을 줄여도 정확도는 떨어지지 않았다.** 같은 30문항을 qwen3:8b로 다시 돌린 결과:
...
달라진 한 건은 `파일구조 관련해서 어떤 내용이 있었지?`로, 거부하던 것이
"2026-05-12에 notion에 등록된 파일구조"라고 답하게 됐다. 그 문서는 실제로
그날 등록돼 있고 본문이 제목 한 줄뿐이라, 없다고 하는 것보다 정확한 답이다.

조각을 줄이는 것이 왜 손해가 아닌지는 검색 쪽 수정과 이어져 있다. 제목이
일치하는 문서를 1순위로 세우고 그 문서의 앞부분을 넘기게 만든 뒤로는, 답에
필요한 근거가 앞쪽에 모인다. 뒤 두 개는 대개 답과 무관한 꼬리였다. 실제로
확인해보니 근거 5개일 때와 3개일 때의 1순위 문서가 시험한 다섯 질문 모두 같았다.

## 모델 비교 (2026-09-07, 같은 12문항·같은 스트리밍 경로)

| | qwen3:8b | gemma3:4b | exaone3.5:2.4b |
|---|---|---|---|
| 모델 크기 | 5.6GB | 3.3GB | 1.6GB |
| 첫 글자까지 평균 | 18.26s | 8.0s (6건) | 4.57s |
| 4초 이내 | 0/12 | 0/6 | 4/12 |

qwen3:8b는 통합 메모리를 5.7GB 가져가 다른 앱과 함께 쓰면 스왑에 눌린다.
그 상태에서는 생성뿐 아니라 프롬프트 읽기까지 느려져 첫 글자가 18초 뒤에 나온다.

**qwen3:4b 기각** — `think: false`가 추론을 끄지 못한다(위 "응답시간 최적화" 항목 참고).

**gemma3:4b 기각** — exaone3.5보다 일관되게 느리고(3건 모두), 날짜 질문에서 같은 실패를 냈다.
"기록에 없습니다"라고 답한 뒤 곧바로 그 날짜를 나열하기까지 했다.

**exaone3.5:2.4b** — 날짜 표기를 고친 뒤 12문항 전부 정답, 거부 0건.
남은 흠은 형식이다. 열두 건 중 일곱 건이 불릿과 굵은 글씨를 썼고, 채팅 UI는 평문으로
그리므로 별표가 그대로 보인다. 예시를 넣어 고쳐보려 했으나 효과가 없었다(위 참고).

## 채팅창은 다른 경로를 탄다 (2026-09-07 수정)

빠른 질문 창·GUI·브라우저 화면은 전부 `presentation/web/app.py`의 SSE 스트림을 거쳐
`AskQuestionResumableUseCase`를 쓴다. `ask.py`와 벤치마크가 쓰는 `AskQuestionUseCase`와는
다른 객체였고, 검색 순서가 양쪽에 복사돼 있어서 갈라졌다.

- **개선이 채팅창에 닿지 않았다.** `promote_document_heads`를 질의 유스케이스에만 넣는 바람에
  답변 거부 7→4 개선분이 정작 사람이 타이핑하는 화면에는 반영되지 않았다.
  이제 두 유스케이스 모두 `services/retrieval_pipeline.py`의 `retrieve()` 하나를 부른다.
- **추론이 켜진 채 버려지고 있었다.** 스트림이 `think=True`로 호출하면서 `on_thinking`은
  넘기지 않아, 추론 토큰을 만들어놓고 화면에 닿기 전에 버렸다. 그동안 사용자는 첫 글자를
  기다렸다. 답변은 근거 조각을 요약하는 일이라 추론이 필요 없어 `think=False`로 바꿨다.

**체감 지연은 첫 글자까지의 시간이다.** 채팅창은 토큰을 흘려보내므로 답변이 다 만들어질
때까지 기다리지 않는다. `kb measure-chat`이 실제 재개형 경로로 그 시간을 재고,
`--think`를 붙이면 추론 생성이 첫 글자를 얼마나 늦추는지 비교할 수 있다.
같은 질문을 두 번 재면 저장된 생성 상태를 이어받아 즉시 끝나므로 매 측정마다 상태를 지운다.
