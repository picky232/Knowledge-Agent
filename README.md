...
1. 대화기록 소스가 세션당 앞 6000자로 잘리는 문제 — 긴 세션은 최근 내용이 인덱스에서 아예 빠짐, 자르는 기준 개선 필요
2. "어제/오늘/최근" 같은 상대 날짜 질의를 감지해 conversation 소스를 우선 검색하는 라우팅 없음
3. 한글-영문 제목 불일치(예: "정글미팅" 질의가 실제 제목 "Jungle Meeting"과 매칭 안 됨) — 키워드 부스트가 언어 간에는 안 통함
4. "자료에 없음" 응답 비율 낮추려면 소스 커버리지(Notion 페이지 공유 범위 등) 확대 검토
5. 화면 캡처(OCR→요약 후 원본 삭제) 소스 추가 여부 결정 — 요약 인덱스 레이어가 준비됐으니 착수 시 같은 패턴으로 얹을 수 있음
