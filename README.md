...
## 응답 속도와 정확도 (2026-09-07 기준)

같은 30문항으로 정확도를, 같은 12문항으로 채팅창 체감 지연을 잰 결과다.
...
무엇을 고쳐서 이렇게 됐는지는 아래 항목들에 하나씩 적어두었다. 요약하면
검색이 엉뚱한 문서를 집던 결함 넷, 채팅창이 벤치마크와 다른 코드를 타던 문제,
화면에 닿지도 않는 추론을 만들던 문제, 그리고 프롬프트에 넣는 근거를 다섯에서
셋으로 줄인 것이다. 측정 자체가 틀렸던 경우도 여러 번 있었고 그것도 적어두었다.

**측정 전에 전원부터 확인할 것.** 저전력 모드에서 잰 같은 구성이 첫 글자 10.9초로
나왔다. 아래 "측정 환경" 항목 참고.
...
## 지금까지의 정확도 (2026-09-07, 같은 30문항, qwen3:8b)

`build_questions`가 seed를 고정하므로 매번 완전히 같은 질문이다.

| 구성 | 답변 거부 | 입력 토큰 |
|---|---|---|
| 기준선 | 7 / 30 | 767 |
| + 제목으로 찾은 문서의 앞부분 제공 | 4 / 30 | 789 |
| + 제목매칭 결함 3건 수정 | 2 / 30 | 816 |
| + 근거 조각 5개 → 3개 | **1 / 30** | **556** |

거부 판정은 여러 번 손봤다. 처음에는 답변 전체에서 문구를 찾다가 정확히 답하고
프롬프트 지시문을 덧붙인 답을 거부로 셌고, 다음에는 문구를 나열하다가 모델이
조사를 바꿔 쓰면 놓쳤다("기록에 근거가 없습니다", "피키 관련한 기록은 없습니다",
"정보는 제공되지 않았습니다"). 지금은 첫 문장에서 "기록/근거/정보/내용" 뒤에
부정이 오는 꼴을 패턴으로 잡는다. 위 숫자는 모두 이 판정으로 다시 매긴 값이다.
...
형식은 프롬프트로 못 고쳐서(예시를 넣어봤으나 효과 없음) 화면에서 마크다운을 지운다.

같은 30문항을 근거 3개로 두 모델에 돌린 결과:

| | qwen3:8b | exaone3.5:2.4b |
|---|---|---|
| 답변 거부 | 1 / 30 | 3 / 30 |
| 첫 글자까지 (검색+프롬프트) | 10.9s | **2.9s** |
| 출력 토큰 평균 | 55 | 119 |
| 320토큰 상한 도달 | 0건 | 5건(상한 200일 때) |

exaone의 세 건을 읽어보면 실제로 진 것은 한 건이다. `피키`는 8b도 거부했고,
`HIGH HOPES ... 언제 만들었어?`에는 "제작 연도는 기록에 없고 기록일은 2026-09-06"이라고
구분해 답해 방문일을 그대로 답한 8b보다 정확했다. 놓친 것은
`프롬프트 엔지니어링 설명 (2026-09-04)` 한 건이다.

**결론: 모델을 바꾸지 않았다.** 위 10.9초는 저전력 모드에서 잰 값이었다.
전원을 연결하고 저전력 모드를 끈 뒤 같은 12문항을 qwen3:8b·근거 3개로 다시 재니:

```
첫 글자까지  평균 2.85s  중앙값 2.72s
첫 글자 4초 이내: 9 / 12
전체 완성까지 평균 6.38s
답변 거부 0건
```

첫 글자 분포는 0.61 · 0.94 · 1.03 · 1.89 · 2.42 · 2.49 · 2.72 · 3.15 · 3.19 · 4.27 · 4.35 · 7.12초다.
정확도를 하나도 내주지 않고 목표를 만족하므로 더 작은 모델로 갈 이유가 없다.
exaone3.5는 받아둔 채로 두었고, `.env`의 `ANSWER_MODEL`만 바꾸면 쓸 수 있다.
